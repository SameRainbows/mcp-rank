...
export function BadgeEmbeds({ serverName, serverSlug }: BadgeEmbedsProps) {
  const badges = [
    ...
    { label: "MCP Rank Verified", value: "Verification badge coming after maintainer claim and evidence review.", disabled: true },
  ];

  async function copyBadge(label: string, value: string, disabled?: boolean) {
    if (disabled) return;
  ...
  return (
    <section className="rounded-lg border border-[var(--arena-line)] bg-white p-5">
      ...
      <p className="mt-2 text-sm leading-6 text-[var(--arena-muted)]">
        Maintainers can share reviewed listing status for {serverName}.
      </p>
      <div className="mt-4 grid gap-2">
        {badges.map((badge) => (
          <button
            ...
            onClick={() => void copyBadge(badge.label, badge.value, badge.disabled)}
